...
try:
    ...
    if os.path.exists(working_dir):
        ...
        # select those picks with stations
        # that have been not been excluded
        # e.g not BH not HN channel types
        ...
        if cfg.sds_url:
            ...
            for i, pick in enumerate(picks):
                _st = sds.get_waveforms(pick.waveform_id['network_code'],
        ...
        # through FDSNWS (if sds_url doesn't exist)
        else:
            logging.info("Requesting waveforms for the associated picks through FDSNWS...")

            for i, pick in enumerate(picks):
                _st = fdsn.get_waveforms(pick.waveform_id['network_code'],
                                        pick.waveform_id['station_code'],
                                        pick.waveform_id['location_code'] or '',
                                        pick.waveform_id['channel_code'][:2]+'?',
                                        starttime,
        ...
        for s in st:
            # get stations' info like lat, lon, dip, azimuth, etc
            inv=fdsn.get_stations(starttime=starttime, 
                                  endtime=endtime, 
            ...
            filename=s.stats.network+'.'+s.stats.station + '.' + \
                     s.stats.location + '.' + s.stats.channel + '.XXXX.XXXXX.SAC'
            ...
            s.stats.sac['stla'] = inv[0][0][0].latitude
            s.stats.sac['stlo'] = inv[0][0][0].longitude
            s.stats.sac['stel'] = inv[0][0][0].elevation
            s.stats.sac['cmpaz'] = inv[0][0][0].azimuth
            s.stats.sac['cmpinc'] = inv[0][0][0].dip
            s.stats.sac['kcmpnm'] = s.stats.channel
            s.stats.sac['scale'] = inv[0][0][0].response.instrument_sensitivity.__dict__['value']
        ...
        try:
            ...
            # cp MWNEAR/calMwext/$event/result_Mw /home/user/MWNEAR/calMwext/$event/$event.mwnear
            ...
            # cp FMNEAR/$event/resmeca /home/user/FMNEAR/$event/$event.fmnear
            ...
            # cp FMNEAR/$event/focmec.ps /home/user/FMNEAR/$event/$event"_focmec.ps"
            ...
            # cp FMNEAR/$event/concatsig.ps /home/user/FMNEAR/$event/$event"_wave.ps"
            ...
            # cp FMNEAR/$event/map.ps /home/user/FMNEAR/$event/$event"_map.ps"
            ...
            # cp FMNEAR/$event/map.ps /home/user/$event/$event"_controlqc"
            ...
            # don't delete event dir, in order to inspect it
